...
These files are generated by `skills/showtime/scripts/analyze_music_cues.py` and are intended for `/showtime` planning. Normal skill runs should read the presets when present and continue without beat/cue sync when they are missing.
